...
import {
  ...
  ShieldCheck, Settings, Eye, EyeOff, Home
} from 'lucide-react'
...
export default function PatientPortal() {
  ...
  // تحميل بيانات العيادة من الـ slug
  useEffect(() => {
    loadClinic()
  }, [clinicSlug])
  ...
  const loadClinic = async () => {
    ...
    if (error || !data) {
      ...
      setClinic(data)
      // استرجاع جلسة المريض (لو موجودة لنفس العيادة)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center gradient-bg-animated p-4">
        <div className="glass rounded-3xl p-12 max-w-md w-full text-center shadow-2xl">
          ...
          <p className="text-gray-600 mb-6">الرابط اللي دخلت عليه غير صحيح</p>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(-45deg, ${primary}, ${primary}cc, #ec4899, #4facfe)`,
    ...
    <div className="min-h-screen flex items-center justify-center p-4 page-enter">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mt-8 space-y-2">
          ...
          <p className="text-white/60 text-xs">🔒 بياناتك محمية ومشفّرة</p>
        </div>
      </div>
    </div>
  )
        ...
        <div className="glass rounded-3xl p-8 shadow-2xl animate-slide-up">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
                  placeholder="05xxxxxxxx" required disabled={loading}
          ...
          <div className="text-center mt-6 pt-6 border-t border-gray-200">
            <p className="text-gray-600 text-sm">
              لسه ما عندكش حساب؟{' '}
              <button onClick={onRegister} className="text-indigo-600 font-bold hover:underline">سجّل الآن</button>
            </p>
          </div>
        </div>
      </div>
...
function RegisterScreen({ clinic, onBack, onSuccess, onLogin }) {
  const [form, setForm] = useState({
    ...
    gender: 'male', blood_type: '', allergies: '', medical_notes: '',
  ...
  const handleRegister = async (e) => {
    ...
    if (form.password !== form.password_confirm) {
      setError('❌ كلمة المرور وتأكيدها غير متطابقين'); return
    }
    ...
    setLoading(true)
    try {
      // التحقق من تكرار رقم الجوال داخل نفس العيادة
      ...
      if (existPhone && existPhone.length > 0) {
        setError('❌ هذا الرقم مسجل بالفعل في هذه العيادة')
      ...
      if (form.national_id.trim()) {
        ...
        if (existId && existId.length > 0) {
          setError('❌ رقم الهوية مسجل بالفعل في هذه العيادة')
          setLoading(false); return
        }
      }
  ...
  const goToStep2 = (e) => {
    ...
    if (!form.name || !form.phone || !form.password) {
      setError('❌ الرجاء ملء كل الحقول الإلزامية'); return
    }
    setStep(2)
  }
        ...
        <div className="glass rounded-3xl p-8 shadow-2xl animate-slide-up">
          ...
          {step === 1 && (
            <form onSubmit={goToStep2} className="space-y-4">
              ...
              <button type="submit" className="w-full py-4 bg-gradient-to-r from-pink-600 to-rose-600 text-white font-bold rounded-2xl btn-glow shadow-xl text-lg">
                التالي ←
              </button>
            </form>
          )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => update('gender', 'male')}
                    className={`py-4 rounded-2xl font-bold transition ${form.gender === 'male' ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-xl scale-105' : 'bg-white/80 text-gray-700 border-2 border-gray-200'}`}>
                    👨 ذكر
                  </button>
                  <button type="button" onClick={() => update('gender', 'female')}
                    className={`py-4 rounded-2xl font-bold transition ${form.gender === 'female' ? 'bg-gradient-to-r from-pink-500 to-rose-600 text-white shadow-xl scale-105' : 'bg-white/80 text-gray-700 border-2 border-gray-200'}`}>
                    👩 أنثى
                  </button>
                </div>
              </div>
          ...
          {step === 1 && (
            <div className="text-center mt-6 pt-6 border-t border-gray-200">
              <p className="text-gray-600 text-sm">
                عندك حساب بالفعل؟{' '}
                <button onClick={onLogin} className="text-pink-600 font-bold hover:underline">سجّل دخول</button>
              </p>
            </div>
          )}
        </div>
...
function Dashboard({ patient, clinic, onLogout, setPatient }) {
  ...
  const loadData = async () => {
    ...
    const [apptsRes, complsRes] = await Promise.all([
      supabase.from('appointments').select('*, doctors(*)')
...
function AppointmentCard({ apt, onCancel }) {
  ...
  const statusLabels = {
    ...
    completed: '✔️ مكتمل', cancelled: '❌ ملغي',
  }
  return (
    <div className="bg-white/60 rounded-2xl p-5 border border-white/40">
      ...
      {onCancel && apt.status !== 'cancelled' && apt.status !== 'completed' && (
        <button onClick={() => onCancel(apt.id)} className="mt-2 text-red-600 hover:text-red-800 text-sm font-bold flex items-center gap-1">
...
function BookAppointmentTab({ patient, clinic, onSuccess }) {
  ...
  const [form, setForm] = useState({ doctor_id: '', appointment_date: '', appointment_time: '', type: 'first_visit', notes: '' })
  const [loading, setLoading] = useState(false)
  ...
  const submit = async (e) => {
    ...
    const { error } = await supabase.from('appointments').insert([{
      clinic_id: clinic.id, patient_id: patient.id, doctor_id: form.doctor_id,
      appointment_date: form.appointment_date, appointment_time: form.appointment_time,
      type: form.type, notes: form.notes, status: 'pending'
    }])
    setLoading(false)
    if (!error) {
      setSuccess(true)
  ...
  return (
    <div className="glass rounded-3xl p-8 shadow-2xl">
      <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
        <Plus className="w-6 h-6 text-indigo-600" /> حجز موعد جديد
      </h3>
      <form onSubmit={submit} className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              ...
              <button key={t.val} type="button" onClick={() => setForm({ ...form, type: t.val })}
                className={`py-3 rounded-2xl font-bold text-sm transition ${form.type === t.val ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg scale-105' : 'bg-white/80 text-gray-700 border-2 border-gray-200'}`}>
        ...
        <Field label="ملاحظات (اختياري)" icon={<FileText className="w-5 h-5" />}>
          <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="أي معلومات إضافية..." rows="3"
...
function ComplaintsTab({ patient, clinic, complaints, onUpdate }) {
  ...
  const submit = async (e) => {
    ...
    await supabase.from('complaints').insert([{
      ...
      subject: form.subject, description: form.description, status: 'open'
    }])
